...
#import "HZQRCodeScanManager.h"
#import <ImageIO/ImageIO.h>

//
NS_ASSUME_NONNULL_BEGIN
@interface HZQRCodeScanManager ()<AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
@property (nonatomic, strong) AVCaptureSession *session;
@property (nonatomic, strong) AVCaptureVideoDataOutput *videoDataOutput;
@property (nonatomic, strong) AVCaptureVideoPreviewLayer *videoPreviewLayer;
@end
NS_ASSUME_NONNULL_END
@implementation HZQRCodeScanManager
static HZQRCodeScanManager *_instance;
...
- (void)setupSessionPreset:(NSString *)sessionPreset metadataObjectTypes:(NSArray *)metadataObjectTypes currentController:(UIViewController *)currentController {
    ...
    if (sessionPreset == nil) {
        NSException *excp = [NSException exceptionWithName:@"HZQRCode" reason:@"setupSessionPreset:metadataObjectTypes:currentController: 方法中的 sessionPreset 参数不能为空" userInfo:nil];
        [excp raise];
    }
    
    if (metadataObjectTypes == nil) {
        NSException *excp = [NSException exceptionWithName:@"HZQRCode" reason:@"setupSessionPreset:metadataObjectTypes:currentController: 方法中的 metadataObjectTypes 参数不能为空" userInfo:nil];
        [excp raise];
    }
    
    if (currentController == nil) {
        NSException *excp = [NSException exceptionWithName:@"HZQRCode" reason:@"setupSessionPreset:metadataObjectTypes:currentController: 方法中的 currentController 参数不能为空" userInfo:nil];
        [excp raise];
    }
...
- (NSString *)readQRCodeImg:(UIImage *)QRCodeImg {
    ...
    //3. 获取扫描结果
    NSString *QRCodeDataStr;
    if (features.count) {
        CIQRCodeFeature *feature = [features objectAtIndex:0];
        QRCodeDataStr = feature.messageString;
    } else {
        QRCodeDataStr = @"暂无信息";
    }
    return QRCodeDataStr;
}
