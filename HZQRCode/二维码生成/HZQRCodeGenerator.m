...
#import "HZQRCodeGenerator.h"

@implementation HZQRCodeGenerator
//常规二维码
+ (UIImage *)generateDefaultQRCodeImageWithData:(NSString *)data imageViewWidth:(CGFloat)imageViewWidth {
    
    CIFilter *imgFilter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    [imgFilter setDefaults];
    
    //
    // 将字符串转换成
    NSData *infoData = [data dataUsingEncoding:NSUTF8StringEncoding];
    [imgFilter setValue:infoData forKeyPath:@"inputMessage"];
    //
    CIImage *outPutImage = [imgFilter outputImage];
    //
    return [self creatImageWithNomalCIImage:outPutImage withSize:imageViewWidth];
}
//logo二维码
...
//-------creatImageWithNomalCIImage--------
+ (UIImage *)creatImageWithNomalCIImage:(CIImage *)image withSize:(CGFloat)size {
